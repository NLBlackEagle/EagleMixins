...
public abstract class MixinRadPotionEffects {
    ...
    @ModifyArg(
            ...
            remap = false
    ) private static int eaglemixins_rawMobBuffDuration(int rawDuration) {
        return 1200;
    }

    @Redirect(
            method = "parseEffects",
            at = @At(value = "INVOKE", target = "Lnc/util/PotionHelper;newEffect(Lnet/minecraft/potion/Potion;II)Lnet/minecraft/potion/PotionEffect;"),
            remap = false
    )
    private static PotionEffect eaglemixins_showParticles(Potion potion, int amplifier, int duration) {
